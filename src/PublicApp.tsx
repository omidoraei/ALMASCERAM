...
import { organizationSchema, websiteSchema, localBusinessSchema, breadcrumbSchema } from './lib/seo/jsonld'
...
function Brand() {
  return (
    <button className="brand" onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })} aria-label="صفحه نخست الماس">
      <span className="brand-mark" aria-hidden="true"><i /><i /><i /><i /></span>
      <span className="brand-copy"><b>الماس</b><small>ALMASCERAM</small></span>
    </button>
  )
}
    ...
    <header className="site-header">
      <div className="header-shell">
        ...
        <div className="header-actions">
          <a className="admin-header-link" href="/admin/login" rel="nofollow" aria-label="ورود به پنل مدیریت الماس">ورود به پنل مدیریت</a>
      ...
      <AnimatePresence>
        {menuOpen && (
          <motion.nav className="mobile-nav" initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }}>
            ...
            <a className="admin-mobile-link" href="/admin/login" rel="nofollow" aria-label="ورود به پنل مدیریت الماس">ورود به پنل مدیریت</a>
          </motion.nav>
        )}
      </AnimatePresence>
    </header>
  ...
  return (
    <article className="product-card">
      <button className="product-image" onClick={() => onDetails(product)} aria-label={`مشاهده جزئیات ${product.name}`}>
        <img src={product.image} alt={`طرح و بافت محصول ${product.name}`} loading="lazy" decoding="async" width="600" height="600" />
  ...
  return (
    <motion.div className="modal-backdrop" role="presentation" onMouseDown={onClose} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
      <motion.div className="product-modal" role="dialog" aria-modal="true" aria-label={`جزئیات ${product.name}`} onMouseDown={(event) => event.stopPropagation()} initial={{ y: 35, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 35, opacity: 0 }}>
        ...
        <div className="modal-image"><img src={product.image} alt={product.name} loading="lazy" decoding="async" width="800" height="800" /><span>{product.badge ?? 'پرسلان معماری'}</span></div>
  ...
  useSEO({
    ...
    jsonLd: [
      ...
      breadcrumbSchema([
        ...
        { name: 'کالکشن‌ها', url: '/#collections' },
        { name: 'راهنمای فنی', url: '/#technical' },
      ]),
    ],
  })
